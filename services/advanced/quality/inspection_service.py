...
from models.qms.supplier_quality import (
    ...
    InspectionPlan, InspectionCharacteristic, InspectionMeasurement
)
...
class InspectionService:
    ...
    def get_pending_inspections(
        ...
        query = self.session.query(InspectionRecord).filter(
            InspectionRecord.result == InspectionResult.PENDING.value
        )

        if work_order_id:
            query = query.filter(InspectionRecord.work_order_id == work_order_id)
        if inspection_type:
            query = query.filter(InspectionRecord.inspection_type == inspection_type)

        inspections = query.order_by(InspectionRecord.created_at).all()
