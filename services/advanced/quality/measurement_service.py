...
class MeasurementService:
    ...
    def calculate_cpk(
        ...
        """
        query = self.session.query(InspectionMeasurement).filter(
            InspectionMeasurement.metric_name == metric_name,
            InspectionMeasurement.target_value.isnot(None),
            InspectionMeasurement.tolerance_plus.isnot(None)
        )

        if part_id:
            query = query.join(InspectionRecord).join(
        ...
        metrics = query.order_by(InspectionMeasurement.created_at.desc()).limit(limit).all()
    ...
    def get_measurement_trend(
        ...
        if inspection_ids:
            query = query.filter(InspectionMeasurement.inspection_id.in_(inspection_ids))
    ...
    def generate_inspection_report(
        ...
        inspection = self.session.query(InspectionRecord).filter(
        ...
        metrics = self.session.query(InspectionMeasurement).filter(
