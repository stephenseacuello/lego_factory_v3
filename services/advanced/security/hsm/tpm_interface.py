...
===========================
By default, this module operates in SIMULATION MODE (config.simulation_mode=True).
In simulation mode, all TPM operations are performed using software cryptography
instead of actual TPM hardware. This is suitable for:
- Development and testing
- Systems without TPM hardware
- Cloud deployments without vTPM

For production environments with TPM hardware:
1. Set TPM_SIMULATION_MODE=false in environment
2. Ensure /dev/tpm0 exists and is accessible
3. Install python-tpm2-pytss library for hardware access

WARNING: Simulation mode does NOT provide the same security guarantees as
hardware TPM. Platform attestation and sealed storage are cryptographically
valid but not hardware-backed.
"""
...
def _get_simulation_mode_default() -> bool:
    """
    Determine simulation mode from environment.

    Returns True (simulation) unless explicitly disabled via environment variable.
    """
    env_value = os.getenv('TPM_SIMULATION_MODE', 'true').lower()
...
class TPMConfig:
    ...
    Attributes:
        device_path: TPM device path (e.g., /dev/tpm0 or /dev/tpmrm0)
        pcr_selection: PCRs to use for sealing (default: [0, 1, 2, 7])
        locality: TPM locality (0-4)
        enable_attestation: Enable remote attestation
        simulation_mode: Use software simulation instead of hardware TPM

    Environment Variables:
        TPM_SIMULATION_MODE: Set to 'false' to use real TPM hardware
        TPM_DEVICE_PATH: Override device path (default: /dev/tpm0)
    ...
    Example:
        ...
        # Or in code
        config = TPMConfig(simulation_mode=False, device_path="/dev/tpmrm0")
    """
    device_path: str = field(default_factory=lambda: os.getenv('TPM_DEVICE_PATH', '/dev/tpm0'))
    pcr_selection: List[int] = field(default_factory=lambda: [0, 1, 2, 7])
    locality: int = 0
    enable_attestation: bool = True
    simulation_mode: bool = field(default_factory=_get_simulation_mode_default)
